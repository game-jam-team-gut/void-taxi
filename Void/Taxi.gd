...
const ROTATION_SPEED = 1.1 #rotation factor
const MINIMAL_SPEED_TO_ROTATE = 55
const MAX_FORWARD_SPEED = 440
const MAX_BACKWARDS_SPEED = 220
...
func get_input():
	...
	if Input.is_action_pressed('ui_right'):
		if(abs(speed) > MINIMAL_SPEED_TO_ROTATE): #rotation depending on the speed, so you can't turn in place
			rotation_dir += 1
